...
namespace FaceQuickstart
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			const string LargeFaceListName = "MyFaceListName";
			...
			FaceListOperations(client, FaceListId, FaceListName, IMAGE_BASE_URL).Wait();
			// Large FaceList - create a large face list, then get data
			LargeFaceListOperations(client, LargeFaceListId, LargeFaceListName, IMAGE_BASE_URL).Wait();
		 ...
		 */
		public static async Task FaceListOperations(IFaceClient client, string listID, string listName, string baseUrl)
		{
			...
			await client.FaceList.CreateAsync(faceListId: listID, name: listName);
			...
			foreach (string image in imageFileNames)
			{
				...
				await client.FaceList.AddFaceFromUrlAsync(faceListId: listID, url: urlFull);
			}
			...
			FaceList faceList = await client.FaceList.GetAsync(listID);
			...
			await client.FaceList.DeleteAsync(listID);
			Console.WriteLine();
		...
		*/
		public static async Task LargeFaceListOperations(IFaceClient client, string listID, string listName, string baseUrl)
		{
			Console.WriteLine("======== LARGE FACELIST OPERATIONS========");
			Console.WriteLine();

			const int timeIntervalInMilliseconds = 1000;
			...
			await client.LargeFaceList.CreateAsync(largeFaceListId: listID, name: listName);
			...
			foreach (string image in singleImages)
			{
				...
				await client.LargeFaceList.AddFaceFromUrlAsync(largeFaceListId: listID, url: $"{baseUrl}{image}");
			}
			...
			await client.LargeFaceList.TrainAsync(listID);

			// Wait for training finish.
			while (true)
			{
				Task.Delay(timeIntervalInMilliseconds).Wait();
				var status = await client.LargeFaceList.GetTrainingStatusAsync(listID);
			...
			Console.WriteLine();
			Parallel.ForEach(
					await client.LargeFaceList.ListFacesAsync(listID),
					faceId =>
					{
			...
			await client.LargeFaceList.DeleteAsync(listID);
			Console.WriteLine();
